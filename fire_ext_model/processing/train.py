...
class Train:
    ...
    def train(self):
        ...
        """
        self.__keras_fun_model.compile(
            optimizers.Adam(), losses.BinaryCrossentropy(), metrics.BinaryAccuracy()
        )
        self.__keras_fun_model.fit(
            self.x_train, self.y_train.astype("int"), 32, 10, validation_split=0.2
        )
